...
router.post("/tas", verifyToken, [
  body('titulo').notEmpty().winthMessage('Titulo é obrigatorio'),
  body('descricao').isLength({ min: 10 }).winthMessage('Descritao deve ter no minimo 10 carecteres')
], async (req, res) => {
...
router.get("/tarefa", async (req, res) => {
  ...
  try {
    const tarefa = await Tarefa.findAll({ limit: parseInt(limit), offset: parseInt(offset) });
    res.json(tarefa);
  } catch (error) {
    console.error("Erro ao criar a tarefa:", error);
    res
