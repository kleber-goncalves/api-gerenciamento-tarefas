...
router.get('/tasks/:id', async (req, res) => {
  try {
    ...
    if (tarefa) {
      logger.info('Tarefa encontrada', { tarefaId: tarefa.id });
      res.json(tarefa);
    } else {
      logger.warn('Tarefa não encontrada', { tarefaId: req.params.id });
      res.status(404).json({ error: 'Tarefas nao encontrada' });
    }
  } catch (error) {
    logger.error('Erro ao buscar tarefa por ID', { error: error.message });
...
router.put('/tasks/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (tarefa) {
      ...
      logger.info('Tarefa atualizada com sucesso', { tarefaId: tarefa.id });
      res.json(tarefa);
    } else {
      logger.warn('Tarefa não encontrada para atualização', { tarefaId: req.params.id });
      res.status(404).json({ error: 'Tarefa nao encontrada id incorreto' });
    }
  } catch (error) {
    logger.error('Erro ao atualizar a tarefa', { error: error.message });
...
router.delete('/tasks/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (tarefa) {
      ...
      logger.info('Tarefa deletada com sucesso', { tarefaId: req.params.id });
      res.json({ message: 'Tarefa deletada com sucesso' });
    } else {
      logger.warn('Tarefa não encontrada para deletar', { tarefaId: req.params.id });
      res.status(404).json({ error: 'Tarefa nao encontrada' });
    }
  } catch (error) {
    logger.error('Erro ao deletar tarefa', { error: error.message });logger.error('Erro ao deletar tarefa', { error: error.message });
