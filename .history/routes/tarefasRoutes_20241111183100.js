...
router.get("/tarefa", async (req, res) => {
  try {
    ...
    console.error("Erro ao criar a tarefa:", error);
    res.status(500).json({ error: "Error ao buscar tarefas", details: error.message });
  }
});
...
router.put("/tarefa/:id", async (req, res) => {
  try {
    ...
    console.error("Erro ao criar a tarefa:", error);
    res.status(500).json({ error: "Error ao atualizar tarefa", details: error.message });
  }
});
...
router.delete("/tarefa/:id", async (req, res) => {
  try {
    ...
    console.error("Erro ao criar a tarefa:", error);
    res.status(500).json({ error: "Error ao deletar tarefa", details: error.message });
  }
});
